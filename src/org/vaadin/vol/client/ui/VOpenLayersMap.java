...
import org.vaadin.vol.client.wrappers.control.NavToolbar;
...
public class VOpenLayersMap extends FlowPanel 
    implements Container, ActionOwner {
    ...
    private GwtOlHandler mouseMoveListener;
    ...
    private int pxWidth = 500;
    private int pxHeight = 500;
    ...
    public void updateFromUIDL(UIDL uidl, final ApplicationConnection client) {
        ...
        if (client.hasEventListeners(this, "mousemove")) {
            if (mouseMoveListener == null) {
                mouseMoveListener = new GwtOlHandler() {

                    public void onEvent(JsArray arguments) {
                       if( (counter++) % 5 == 0){
                           JsObject event = arguments.get(0).cast();
                           LonLat lonlat = map.getLonLatFromPixel( (Pixel)event.getFieldByName("xy").cast());
                           
                           client.updateVariable(paintableId, "lon", lonlat.getLon(), false);
                           client.updateVariable(paintableId, "lat", lonlat.getLat(), false);
                           client.updateVariable(paintableId, "mousemove", "", true); 
                       }
                    }
                };
                getMap().registerEventHandler("mousemove", mouseMoveListener);
            }
        ...
        if (uidl.hasAttribute("extentLayer")) {
           NavToolbar.removeControl(navToolBarControl, df);
           NavToolbar.removeControl(navToolBarControl, tf);

           extentLayer = (VVectorLayer) uidl.getPaintableAttribute("extentLayer",  client);
           df = extentLayer.getDrawFeature();
           tf = extentLayer.getTransformFeature();    
           NavToolbar.addControl(navToolBarControl, df);
           NavToolbar.addControl(navToolBarControl, tf);
        }       
        else if(uidl.hasAttribute("removeExtentLayer")){
            NavToolbar.removeControl(navToolBarControl, df);
            NavToolbar.removeControl(navToolBarControl, tf);
        }
        
        if (uidl.hasAttribute("activateExtentTransform"))
        {
        	NavToolbar.activateControl(navToolBarControl, tf);

            Iterator<Widget> iterator = extentLayer.iterator();
    ...
    private VVectorLayer extentLayer = null;
    private DrawFeature df = null;
    private TransformFeature tf = null;
    private NavToolbar navToolBarControl = null;
            ...
            for (int i = 0; i < controls.length; i++) {
                ...
                if (oldcontrols.contains(name)) {
                    ...
                    if (controlByName != null) {
                       ...
                       if(name.compareToIgnoreCase("NavToolbar") == 0){
                          navToolBarControl = (NavToolbar) controlByName;
                       }
